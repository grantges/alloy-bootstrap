!function(global, factory) {
    ...
    function configFromStringAndFormat(config) {
        ...
        true === getParsingFlags(config).bigHour && config._a[HOUR] <= 12 && config._a[HOUR] > 0 && (getParsingFlags(config).bigHour = void 0);
    ...
    function prepareConfig(config) {
        ...
        isArray(format) ? configFromStringAndArray(config) : format ? configFromStringAndFormat(config) : isDate(input) ? config._d = input : configFromInput(config);
        return config;
    }
